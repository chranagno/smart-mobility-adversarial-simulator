...
class SumoTLManagerNull(object):
    """Fallback traffic light manager that performs no operations."""
    def get_all_landmarks(self):
        return set()
    def get_state(self, landmark_id):
        return None
...
class SumoSimulation(object):
    ...
    """
    def __init__(self, cfg_file, step_length, host=None, port=None, sumo_gui=False, client_order=2,num_clients=1, skip_tls=False):
        if sumo_gui is True:
        ...
        if host is None or port is None:
            ...
            traci.init(
                port=port,
                host=host,
                traceFile="/home/chranagno/Workspace/repos/simulator_paper/traci.log",
                traceGetters=True,        # include getter calls in the log
            )
        ...
        # Track the list of actors currently active in the simulation.
        ...
        if skip_tls:
            logging.warning('Skipping SUMO traffic light manager (skip_tls=True)')
            self.traffic_light_manager = SumoTLManagerNull()
        else:
            try:
                self.traffic_light_manager = SumoTLManager()
            except Exception as exc:
                logging.warning('Failed to initialize SUMO traffic light manager, disabling it: %s', exc)
                self.traffic_light_manager = SumoTLManagerNull()
    ...
    def destroy_actor(actor_id):
        ...
        """
        try:
            # Try to remove the actor directly - avoid getIDList() as it can block in multi-client setups
            traci.vehicle.unsubscribe(actor_id)
            traci.vehicle.remove(actor_id)
        except traci.TraCIException:
            # Actor may not exist or may have already been removed
            pass
    ...
    def synchronize_vehicle(self, vehicle_id, transform, signals=None):
        ...
        try:
            ...
            logging.error('Error moving vehicle %s to position: %s, %s | Error: %s', vehicle_id, loc_x, loc_y, e)
            return False
        # traci.vehicle.moveToXY(vehicle_id, "", 0, loc_x, loc_y, angle=yaw, keepRoute=4,matchThreshold=10000)
    ...
    def barrier_step(self):
        """
        Execute a single TraCI simulation step. This satisfies SUMO's multi-client barrier.
        """
        traci.simulationStep()

    def _process_step_results(self):
        """
        Update cached state after a TraCI step has completed.
        """
        # self.traffic_light_manager.tick()

        # Use non-blocking methods to get spawned and destroyed actors
        # These methods are safe to call in multi-client TraCI setups
        self.spawned_actors = set(traci.simulation.getDepartedIDList())
        self.destroyed_actors = set(traci.simulation.getArrivedIDList())
        
        # Note: We avoid traci.vehicle.getIDList() here as it can block in multi-client setups
        # Instead, we maintain _active_actors by tracking spawned/destroyed actors

    def tick(self, external_step=False):
        """
        Tick the SUMO simulation, optionally assuming the TraCI barrier step was executed externally.
        """
        if not external_step:
            self.barrier_step()
        self._process_step_results()

        # Update active actors set
        self._active_actors.difference_update(self.destroyed_actors)
        self._active_actors.update(self.spawned_actors)
