...
class SimulationSynchronization(object):
    ...
    def __init__(self,
        ...
        self.carla_master = carla_master
        ...
        traffic_manager = self.carla.client.get_trafficmanager(8001)
        traffic_manager.set_synchronous_mode(True)

    def tick(self, external_sumo_step=False):
        """
        Tick to simulation synchronization
        """
        if self.carla_master:
            return self._tick_carla_master(external_sumo_step=external_sumo_step)
        ...
        self.sumo.tick(external_step=external_sumo_step)
        ...
        print(f"sumo_spawned_actors: {sumo_spawned_actors}")
        ...
        # print(f"sumo_destroyed_actors: {self.sumo.destroyed_actors}")
        ...
        # print(f"sumo2carla_ids: {self.sumo2carla_ids}")
        ...
        # print(f"carla_traffic_light_ids: {self.carla.traffic_light_ids}")
        ...
        # carla-->sumo sync
        # -----------------
        # print(f"carla_tick_start")
        # print(f"carla_tick")
        self.carla.tick()
        # print(f"carla_tick_done")
        # Spawning new carla actors (not controlled by sumo)
        
        carla_spawned_actors = self.carla.spawned_actors - set(self.sumo2carla_ids.values())
        # print(f"carla_spawned_actors: {carla_spawned_actors}")
        ...
        # print(f"carla_destroyed_actors: {self.carla.destroyed_actors}")
        ...
        # print(f"carla2sumo_ids: {self.carla2sumo_ids}")
        ...
        # print(f"sumo_traffic_light_ids: {self.sumo.traffic_light_ids}")
    ...
    def _tick_carla_master(self, external_sumo_step=False):
        """
        Synchronization loop when CARLA is the source of truth.
        All actors are spawned in CARLA and mirrored to SUMO.
        """
        self.sumo.tick(external_step=external_sumo_step)
        ...
        self.carla.refresh_actor_cache()

        for carla_actor_id in self.carla.destroyed_actors:
            if carla_actor_id in self.carla2sumo_ids:
                self.sumo.destroy_actor(self.carla2sumo_ids.pop(carla_actor_id))

        for carla_actor_id in self.carla.spawned_actors:
            if carla_actor_id in self.carla2sumo_ids:
                continue
            carla_actor = self.carla.get_actor(carla_actor_id)
            type_id = BridgeHelper.get_sumo_vtype(carla_actor)
        ...
        for carla_actor_id in list(self.carla2sumo_ids):
            sumo_actor_id = self.carla2sumo_ids[carla_actor_id]
            carla_actor = self.carla.get_actor(carla_actor_id)
            sumo_actor = self.sumo.get_actor(sumo_actor_id)
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--carla-master',
                           action='store_true',
                           help='mirror CARLA actors into SUMO only (CARLA spawns all actors)')
